...
namespace ThirdYearLP_IP_Solver.Algorithms
{
    ...
    internal class Sensitivity_Analysis
    {
        ...
        public void DoPreliminaries(List<double> objFunc, List<List<double>> constraints, bool isMin)
        {
            ...
            {
                OutputWriter.Write($"{headerStr[i],15} ");
            }
            OutputWriter.WriteLine();
            for (int i = 0; i < firstTab.Count; i++)
            {
                ...
                {
                    OutputWriter.Write($"{firstTab[i][j],15:F4} ");
                }
                OutputWriter.WriteLine();
            }
            ...
            for (int i = 0; i < revisedTab.Count; i++)
            {
                for (int j = 0; j < revisedTab[i].Count; j++)
                {
                    OutputWriter.Write($"{revisedTab[i][j],15:F4} ");
                }
                OutputWriter.WriteLine();
            }
        ...
        public (double?, double?) GetNonBasicVariableRange(int varIndex)
        {
            if (revisedTab == null)
            {
                OutputWriter.WriteLine("Please run the optimization first!");
                return (null, null);
            }
            ...
            OutputWriter.WriteLine($"Current value: 0");
            OutputWriter.WriteLine($"Variable column: [{string.Join(", ", varColumn.Select(v => v.ToString("F4")))}]");
            OutputWriter.WriteLine($"RHS values: [{string.Join(", ", rhsValues.Select(v => v.ToString("F4")))}]");

            if (maxIncrease == double.PositiveInfinity)
            {
                OutputWriter.WriteLine("Range: [0, +∞) - No upper bound (unbounded)");
            }
            else
            {
                OutputWriter.WriteLine($"Range: [0, {maxIncrease:F4}] - Limited by feasibility constraints");
            }
        ...
        public void ApplyNonBasicVariableChange(int varIndex, double newValue)
        {
            ...
            if (basicVarSpots.Contains(varIndex))
            {
                OutputWriter.WriteLine($"Variable {headerStr[varIndex]} is a basic variable!");
                return;
            }

            if (newValue < 0)
            {
                OutputWriter.WriteLine("Non-basic variable value cannot be negative!");
                return;
            }

            OutputWriter.WriteLine($"\nApplying change to Non-Basic Variable {headerStr[varIndex]}:");
            OutputWriter.WriteLine($"Changing from 0 to {newValue}");
            ...
            for (int i = 0; i < basicVarSpots.Count; i++)
            {
                ...
                OutputWriter.WriteLine($"{headerStr[basicVarSpots[i]]}: {oldValue:F4} → {newValueCalc:F4}");
            }
        ...
        public (double?, double?) GetBasicVariableRange(int varIndex)
        {
            ...
            if (!basicVarSpots.Contains(varIndex))
            {
                try
                {
                    OutputWriter.WriteLine($"Variable {headerStr[varIndex]} is not a basic variable!");
                }
                catch
                {
                    return (null, null);
            ...
            OutputWriter.WriteLine($"Current value: {currentValue:F4}");
            OutputWriter.WriteLine($"B^-1 row: [{string.Join(", ", bInvRow.Select(v => v.ToString("F4")))}]");
            OutputWriter.WriteLine($"Range: [{lowerBound:F4}, {(upperBound == double.PositiveInfinity ? "∞" : upperBound.ToString("F4"))}]");
        ...
        public void ApplyBasicVariableChange(int varIndex, double newValue)
        {
            ...
            if (newValue < 0)
            {
                OutputWriter.WriteLine("Basic variable value cannot be negative in standard form!");
                return;
            }
            ...
            OutputWriter.WriteLine($"Changing from {currentValue:F4} to {newValue}");
            OutputWriter.WriteLine("Note: Changing a basic variable directly may affect optimality.");
            OutputWriter.WriteLine("This would require re-solving or using parametric programming techniques.");
        }
        ...
        public (double?, double?) GetRHSRange(int constraintIndex)
        {
            ...
            {
                OutputWriter.WriteLine("Invalid constraint index!");
                return (null, null);
            }
            ...
            OutputWriter.WriteLine($"Current RHS value: {currentRhs:F4}");
            OutputWriter.WriteLine($"B^-1 column {constraintIndex}: [{string.Join(", ", bInvColumn.Select(v => v.ToString("F4")))}]");
            OutputWriter.WriteLine($"Current basic values: [{string.Join(", ", currentBasicValues.Select(v => v.ToString("F4")))}]");
            OutputWriter.WriteLine($"Allowable decrease: {(maxDecrease == double.PositiveInfinity ? "∞" : maxDecrease.ToString("F4"))}");
            OutputWriter.WriteLine($"Allowable increase: {(maxIncrease == double.PositiveInfinity ? "∞" : maxIncrease.ToString("F4"))}");
            OutputWriter.WriteLine($"Range: [{(lowerBound == double.NegativeInfinity ? "-∞" : lowerBound.ToString("F4"))}, {(upperBound == double.PositiveInfinity ? "∞" : upperBound.ToString("F4"))}]");
        ...
        public void ApplyRHSChange(int constraintIndex, double newRHS)
        {
            ...
            OutputWriter.WriteLine($"Changing RHS from {currentRhs:F4} to {newRHS}");
            OutputWriter.WriteLine($"Change amount: {change:F4}");
            ...
            if (constraintIndex < matrixBNegOne[0].Count)
            {
                ...
                for (int i = 0; i < basicVarSpots.Count; i++)
                {
                    ...
                    OutputWriter.WriteLine($"{headerStr[basicVarSpots[i]]}: {oldValue:F4} → {newValue:F4}");
                }
                ...
                OutputWriter.WriteLine($"Objective function: {currentObj:F4} → {newObj:F4} (change: {objChange:F4})");
            }
        }
        ...
        public (double?, double?) GetNonBasicColumnRange(int varIndex, int rowIndex)
        {
            ...
            {
                OutputWriter.WriteLine("Invalid row index!");
                return (null, null);
            }
            ...
            OutputWriter.WriteLine($"Current value: {currentValue:F4}");
            OutputWriter.WriteLine($"Current reduced cost: {currentReducedCost:F4}");
            OutputWriter.WriteLine($"Corresponding cbv coefficient: {cbvCoeff:F4}");
            OutputWriter.WriteLine($"B^-1 row {rowIndex}: [{string.Join(", ", bInvRow.Select(v => v.ToString("F4")))}]");
            OutputWriter.WriteLine($"Range: [{(lowerBound == double.NegativeInfinity ? "-∞" : lowerBound.ToString("F4"))}, {(upperBound == double.PositiveInfinity ? "∞" : upperBound.ToString("F4"))}]");
        ...
        public void ApplyNonBasicColumnChange(int varIndex, int rowIndex, double newValue)
        {
            ...
            OutputWriter.WriteLine($"Changing from {currentValue:F4} to {newValue}");
            OutputWriter.WriteLine($"Change amount: {change:F4}");
            OutputWriter.WriteLine("This requires updating the tableau and checking optimality.");
        }

        public void DisplayCurrentTableau()
        {
            OutputWriter.WriteLine("\nCurrent Optimal Tableau:");
            OutputWriter.WriteLine(new string('-', 50));
            for (int i = 0; i < headerStr.Count; i++)
            {
                OutputWriter.Write($"{headerStr[i],15} ");
            }
        ...
        public void HandleNonBasicVariableRange(int varIdx = -1)
        {
            ...
            for (int i = 0; i < headerStr.Count - 1; i++)  // Exclude RHS
            {
                ...
                OutputWriter.WriteLine($"{i}: {headerStr[i]} ({status})");
            }
        ...
        public void HandleNonBasicVariableChange(float val = -1, int varIdx = -1)
        {
            ...
            for (int i = 0; i < headerStr.Count - 1; i++)
            {
                if (!basicVarSpots.Contains(i))
                {
                    OutputWriter.WriteLine($"{i}: {headerStr[i]}");
                }
            }
            ...
            if (varIdx != -1 && val != -1)
            {
                OutputWriter.WriteLine("Enter new value: ");
                ...
                {
                    OutputWriter.WriteLine("Invalid variable index!");
                }
            }
            else
            {
                OutputWriter.WriteLine("Enter variable index: ");
                OutputWriter.WriteLine("Please enter a valid number!");
            }
        }
        ...
        public void HandleBasicVariableRange(int varIdx = -1)
        {
            ...
            for (int i = 0; i < basicVarSpots.Count; i++)
            {
                ...
                OutputWriter.WriteLine($"{varIdxL}: {headerStr[varIdxL]}");
            }
        ...
        public void HandleBasicVariableChange(float val = -1, int varIdx = -1)
        {
            ...
            for (int i = 0; i < basicVarSpots.Count; i++)
            {
                ...
                OutputWriter.WriteLine($"{varIdxL}: {headerStr[varIdxL]} (current: {currentVal:F4})");
            }
        ...
        public void HandleRHSRange(int constraintIdx = -1)
        {
            ...
            for (int i = 0; i < revisedTab.Count - 1; i++)
            {
                ...
                OutputWriter.WriteLine($"Constraint {i}: Current RHS = {currentRhs:F4}");
            }
            ...
            {
                OutputWriter.WriteLine("Enter constraint index: ");
                OutputWriter.WriteLine("Please enter a valid number!");
            }
        ...
        public void HandleRHSChange(float val = -1, int varIdx = -1)
        {
            ...
            if (varIdx != -1 && val != -1)
            {
                ...
                OutputWriter.Write("Enter new RHS value: ");
                if (varIdx >= 0 && varIdx < headerStr.Count - 1)
                {
                    ApplyBasicVariableChange(varIdx, val);
                }
            ...
            if (varIdx != -1 && rowIdxIn != -1)
            {
                if (!basicVarSpots.Contains(varIdx))
                {
                    ...
                    for (int i = 0; i < revisedTab.Count - 1; i++)
                    {
                        ...
                        OutputWriter.WriteLine($"Row {i}: {currentVal:F4}");
                    }

                    OutputWriter.Write("Enter row index: ");
                    if (rowIdxIn != -1)
                    {
                ...
                {
                    OutputWriter.WriteLine("Selected variable is basic!");
                }
            }
            else
            {
                OutputWriter.Write("Enter non-basic variable index: ");
                OutputWriter.WriteLine("Please enter a valid number!");
            }
        }
        ...
        public void HandleNonBasicColumnChange(int rowIdx = -1, float val = -1, int varIdx = -1)
        {
            ...
            if (varIdx != -1 && rowIdx != -1 && val != -1)
            {
                if (!basicVarSpots.Contains(varIdx))
                {
                    ...
                    if (rowIdx != -1)
                    {
                        OutputWriter.Write("Enter new coefficient value: ");
                        if (val != -1)
                        {
            ...
            {
                OutputWriter.WriteLine("Enter non-basic variable index: ");
                OutputWriter.WriteLine("Please enter a valid number!");
            }
        }
        ...
        List<double> activity)
        {
            ...
            OutputWriter.WriteLine("Display Column: " + string.Join(", ", displayCol));
        ...
        public void PrintTableau(List<List<double>> tableau, string title = "Tableau")
        {
            ...
            if (isConsoleOutput)
            {
                ...
                for (int i = 0; i < tableau.Count; i++)
                {
                    ...
                    {
                        OutputWriter.Write($"{RoundValue(tableau[i][j]),8:0.0000}  ");
                    }
                    OutputWriter.WriteLine();
                }
                OutputWriter.WriteLine();
        ...
        public (List<List<double>>, List<List<double>>) DoAddConstraint(List<List<double>> addedConstraints, List<List<double>> overRideTab = null)
        {
            ...
            {
                OutputWriter.WriteLine("needs an input table");
                return (null, null);
            }
